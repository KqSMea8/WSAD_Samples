 ...
 * @author: ILS User
 */
public class TimeSheetMapperTest extends TestCase {
	...
	public void testCreateUpdateRemove() throws MappingException {
		...
		createId = newTimeSheet.getTimesheetID();
	...
	public void tearDown() {
		...
		if (createId != -1) {
			TimeSheet newTimeSheet = buildTestTimeSheet();
			newTimeSheet.setTimesheetID(createId);
			try {
				newTimeSheet.getMapper().remove(newTimeSheet);
			} catch (MappingException me) {
			}
		}
	}
	...
	public void testFindAll() throws MappingException {
		...
		assertTrue("Expected at least 2 TimeSheets from findAll", tsList.size() >= 2);
	}
	...
	public void testFindByKey() throws MappingException {
		createId = -1;
		TimeSheetMapper mapper = new com.wsbook.casestudy.mapping.jdbc.TimeSheetMapper();
		TimeSheet ts = mapper.findByKey("A100", "12/15/2001");
		// retrieving entries for John Doe
